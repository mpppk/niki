...
1. **結果へのリンクを足す。** ユーザーの文言には触れない。
2. **指示行をキューから外す。** 外し方は、その行が実行後も情報を持つかで 2 通り（下記）。

どちらの道でもキューは空へ向かう。子行の有無で完了を判定する案もあるが、
それだと lint のたびに被リンク全ページの本文を読むことになり、ページ数に比例して重くなる。

#### 実行後も情報を持つ指示 — 平文化する

**マーカーのブラケットを外して平文にする**（`[query]` → `query`）。
これでキューから消える。記号だけを変え、意味は変えない。結果へのリンクは子行として足す。
...
**「2026 以降の動きを調べて」という問いの立て方は、結果ページのどこにも残らない。**
なぜその調査をしたのかがページの文脈に付いたまま残ることがこの記法の価値なので（§11 冒頭）、
ユーザーの言葉が残っている限り行は消さない。

#### 実行後に情報を持たない指示 — 行ごと削除する

**マーカーと署名以外に何も無い指示、あるいは残りの内容がすべて結果ページに移る指示は、
実行後に行ごと削除する。** 平文化しても `ingest [yuki.icon]` のような残骸が残るだけで、
ページを読む人間にも次の lint にも何も伝えない。

判定は 1 つ。**その行から操作名と `[yuki.icon]` を除いた残りが、実行後の wiki のどこにも
無いものを含むか。** 含むなら平文化、含まないなら削除する。

- `[ingest] [yuki.icon]`（そのページ自体の取り込み依頼）→ 削除。残りが空。
- `[ingest] https://example.com/foo [yuki.icon]` → 削除。URL は作った summary の
  Infobox `url` が持つ（§7）ので、行を消しても失われない。
- `[lint] [yuki.icon]` → 削除。残りが空。
- `[query] このページと OAuth の関係は？ [yuki.icon]` → 平文化。問いが wiki に残らない。

**子行にユーザーの補足がぶら下がっている指示は、親行が `[ingest]` だけでも削除しない**（§11 記法）。
親行を消すと補足が宙に浮く。この場合は平文化し、結果リンクを補足と同じ深さの子行として足す。

**削除するときは、子行に足すはずだった結果へのリンクを指示行の位置に繰り上げる。**
署名の付かない普通の wiki 本文になる。指示行ごと結果リンクまで消すと、
そのページから結果ページへの導線が切れ、結果ページが孤立しかねない（§4）。

```
（実行前）
[ingest] https://example.com/foo [yuki.icon]

（実行後）
[Foo の設計思想]
```

**ユーザーの行に対して許されるのは、この平文化と削除の 2 つだけである。**
文言の修正・要約・部分的な書き換えはしない。削除は行を丸ごと消すことであって、
ユーザーの言葉を残したまま削り込むことではない。**迷ったら平文化を選ぶ。**
消しすぎは取り返せないが、残骸は次の lint で拾える。
...
8. 日付ページに 1 行追記する。
9. wiki 上の指示から始まった取り込みなら、依頼行に結果を返して指示行を片付ける。
   `[ingest]` と URL しか無い依頼行は、結果リンクを繰り上げて行ごと消す（§11）。
...
5. wiki 上の指示に答えた場合は、依頼行に結果を返して指示行を片付ける。
   問いが残る指示は平文化し、操作名しか無い指示は結果リンクを繰り上げて行ごと消す（§11）。
...
- ユーザーの発言行の書き換え（マーカーの平文化と、情報を持たない指示行の削除のみ例外。§11）
